...
class BaseballGame:
    def __init__(self, root):
        ...
        minimize_btn = tk.Button(
            self.top_frame, text="_", command=self.minimize_game, font=("Arial", 12)
        )
        minimize_btn.pack(side="right", padx=5, pady=5)
        close_btn = tk.Button(
            self.top_frame, text="X", command=self.close_game, font=("Arial", 12)
        )
    ...
    def setup_main_menu(self):
        ...
        self.canvas = tk.Canvas(
            self.root, width=self.screen_width, height=self.screen_height, bg="green"
        )
        ...
        self.canvas.create_polygon(
            [home_pt, first_pt, second_pt, third_pt], outline="white", fill="", width=3
        )
        ...
        for i, opt in enumerate(self.menu_options):
            ...
            txt = self.canvas.create_text(
                cx, y_pos, text=opt, font=opt_font, fill="black"
            )
    ...
    def draw_menu_bases(self):
        ...
        roof = bs / 2
        pts = [
            field_coords["home"][0] - half,
            field_coords["home"][1] + half,
            field_coords["home"][0] + half,
            field_coords["home"][1] + half,
            field_coords["home"][0] + half,
            field_coords["home"][1] - half,
            field_coords["home"][0],
            field_coords["home"][1] - half - roof,
            field_coords["home"][0] - half,
            field_coords["home"][1] - half,
        ]
        self.canvas.create_polygon(
            pts, outline="white", fill="white", width=3, tags="menu_base"
        )
    ...
    def on_return_release(self, event):
        hold_duration = (
            time.time() - self.last_return_press_time
            if self.last_return_press_time
            else 0
        )
    ...
    def show_pause_menu(self):
        ...
        title = tk.Label(
            self.pause_window,
            text="Paused",
            font=("Arial", int(50 * self.base_scale)),
            bg="darkgray",
            fg="white",
        )
        title.pack(pady=50)
    ...
    def draw_field(self):
        ...
        self.canvas.create_line(
            self.field_coords["home"][0],
            self.field_coords["home"][1],
    ...
    def draw_bases(self):
        ...
        for b in ["home", "first", "second", "third"]:
            ...
            if b == "home":
                ...
                self.canvas.create_polygon(
                    pts, outline="white", fill="white", width=3, tags="base"
                )
            else:
                col = "white"
                if self.bases[b]:
    ...
    def update_batter_counter_display(self):
        ...
        self.canvas.create_text(
            self.screen_width - 150 * self.base_scale,
            self.screen_height - 100 * self.base_scale,
            text=txt,
            font=font,
            fill="white",
            tags="batter_counter",
    ...
    def announce_half_inning(self):
        ann = (
            f"Top of the {self.ordinal(self.current_inning)} inning."
            if self.half == "top"
            else f"Bottom of the {self.ordinal(self.current_inning)} inning."
        )
        outs_txt = {0: "zero outs", 1: "one out", 2: "two outs", 3: "three outs"}[
            self.outs
        ]
    ...
    def simulate_computer_pitch(self):
        self.selected_pitch_type = random.choice(
            ["Fastball", "Curveball", "Slider", "Knuckleball", "Changeup"]
        )
    ...
    def process_batting_selection(self, selected):
        ...
        if selected in ["Steal 2nd Base", "Steal 3rd Base"]:
            ...
            self.speak(
                ...
                + (
                    "successful."
                    if outcome == "Steal Success"
                    else "failed. Runner is out."
                )
            )
            terminal = True
        elif selected == "Bunt":
            ...
            if selected == "Hold":
                outcome = (
                    "Ball"
                    if self.selected_pitch_location == "Outside"
                    else ("Ball" if random.uniform(0, 100) <= 60 else "Strike")
                )
            elif selected == "Power Swing":
                if self.selected_pitch_type == "Fastball":
                    outcome = self.weighted_choice(
                        {"Strike": 60, "Pop Fly Out": 30, "HR": 10}
                    )
                else:
                    outcome = self.weighted_choice(
                        {
            ...
            if outcome == "Foul":
                ...
                self.update_batter_counter_display()
                self.draw_pitch_marker(
                    self.selected_pitch_location, color="red"
                )  # Ensure red indicator shows
    ...
    def update_bases(self, outcome, batter):
        if outcome == "Single":
            ...
            if self.bases["third"]:
                self.score[
                    self.away_team if self.half == "top" else self.home_team
                ] += 1
    ...
    def animate_ball_landing(self, outcome, callback):
        ...
        if outcome == "Foul":
            ...
            while True:
                    ...
                    random.randint(
                        int(self.field_coords["third"][0]),
                        int(self.field_coords["first"][0]),
                    ),
                    random.randint(
                        int(self.field_coords["second"][1]),
                        int(self.field_coords["home"][1]),
                    ),
                )
        ...
        self.canvas.create_line(
            start[0], start[1], end[0], end[1], fill="blue", width=2, tags="blue_line"
        )
    ...
    def end_game(self):
        ...
        self.canvas.create_text(
            ...
            text=result_text,
            font=("Arial", int(80 * self.base_scale), "bold"),
            fill=result_color,
        )
